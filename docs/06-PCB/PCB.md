---
title: PCB
---

## Overview

This PCB is designed to support a value, PIC18(L)F4XK42 Microcontroller and a Temperature Sensor.


## PCB 3D-View
![Front View](314PCB.png)

**Figure #1:** Showing Zane's Subsystem PCB Front.

![Back View](314PCBack.png)

**Figure #2:** Showing Zane's Subsystem PCB Rear.

 The PCB GERBER files can be accessed [*here*](Last_PCB.zip). The PCB Project files can be accessed [*here*](Updated_PCB.zip).
